...
def iterate_x(x, timestep):
    ...
    global thrust_moment,R_iris2,R_imu,m,F,e3,acc_imu,acc,debug_tmp,f_vector,allo_mat,allo_mat_inv,rpy
    ret = np.zeros(len(x))
    # dynamics
    a = thrust_moment[0]*np.dot(R_iris2,e3)/m - F/m 
...
def pos_rpy_cb(data):
    ...
    global sensor_data,rpy,R_iris2,debug_tmp
    ...
    R_iris2 = quaternion.rotation_matrix
...
if __name__ == "__main__":
    try:
        rospy.init_node('UKF_iris2')
        state_pub = rospy.Publisher("/estimated_state", Float64MultiArray, queue_size=10)
        rpy_pub = rospy.Publisher("/iris2/rpy", Float64MultiArray, queue_size=10)
        debug_pub = rospy.Publisher("/debug", Float64MultiArray, queue_size=10) 
        rospy.Subscriber("/iris2/ground_truth/odometry", Odometry, pos_rpy_cb, queue_size=10)
        rospy.Subscriber("/iris2_control_input", Odometry, thrust_moment_cb, queue_size=10)
        rospy.Subscriber("/payload_joint2_ft_sensor", WrenchStamped, ft_cb, queue_size=10)
        rospy.Subscriber("/iris2/ground_truth/imu", Imu, imu_cb, queue_size=10)
        rospy.Subscriber("/estimated_state", Float64MultiArray, debug_cb, queue_size=10)
        rospy.Subscriber("/iris2_original_rotor_vel", Float64MultiArray, rotors_cb, queue_size=10)
